...
void edm4eic::Helix::setCurvature(double val)
{
    if (val < 0) {
	mCurvature = -val;
	mH = -mH;
...
void edm4eic::Helix::setPhase(double val)
{
    mPhase       = val;
...
void edm4eic::Helix::setDipAngle(double val)
{
    mDipAngle    = val;
...
double edm4eic::Helix::xcenter() const
{
    if (mSingularity)
	return 0;
    else
...
double edm4eic::Helix::ycenter() const
{
    if (mSingularity)
	return 0;
    else
...
double edm4eic::Helix::fudgePathLength(const edm4hep::Vector3f& p) const
{
    double s;
...
double edm4eic::Helix::distance(const edm4hep::Vector3f& p, bool scanPeriods) const
{
    return edm4hep::utils::magnitude(this->at(pathLength(p,scanPeriods))-p);
}

double edm4eic::Helix::pathLength(const edm4hep::Vector3f& p, bool scanPeriods) const 
{
    //
...
double edm4eic::Helix::period() const
{
    if (mSingularity)
...
std::pair<double, double> edm4eic::Helix::pathLength(double r) const
{
...
std::pair<double, double> edm4eic::Helix::pathLength(double r, double x, double y)
{
    double x0 = mOrigin.x;
    double y0 = mOrigin.y;
    mOrigin.x = x0-x;
...
double edm4eic::Helix::pathLength(const edm4hep::Vector3f& r,
...
edm4eic::Helix::pathLengths(const Helix& h, double minStepSize, double minRange) const
{
    //
...
void edm4eic::Helix::moveOrigin(double s)
{
    if (mSingularity)
	mOrigin	= at(s);
    else {
...
int operator== (const edm4eic::Helix& a, const edm4eic::Helix& b)
{
    //
...
int operator!= (const edm4eic::Helix& a, const edm4eic::Helix& b) {return !(a == b);}

std::ostream& operator<<(std::ostream& os, const edm4eic::Helix& h)
{
    return os << "("
